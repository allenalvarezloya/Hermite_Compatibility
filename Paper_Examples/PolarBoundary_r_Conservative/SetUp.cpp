...
void SetUp::compute_initial_data(Darray4 &u,int nr_start,int nr_end,int ns_start,int ns_end,Darray1 &rr,Darray1 &ss,double time){
    Darray2 U_loc;
    ...
    for(int j = ns_start; j<= ns_end; j++){
        for(int i = nr_start;i<=nr_end; i++){
            r_loc = rr(i);
            s_loc = ss(j);
            indat_(U_loc_ptr,&r_loc,&s_loc,&hr,&hs,&m,&I,&time);
...
void transformation_x(int m,int nr_start,int ns_start,int nr, int ns, 
    ...
    for(int j = ns_start; j<= ns; j++){
        for(int i = nr_start;i<=nr; i++){
            r_loc = r(i);
            s_loc = s(j);
            indat_(X_loc_ptr,&r_loc,&s_loc,&hr,&hs,&m2p5,&I,&time);
...
void transformation_y(int m,int nr_start,int ns_start,int nr, int ns, 
    ...
    for(int j = ns_start; j<= ns; j++){
        for(int i = nr_start;i<=nr; i++){
            r_loc = r(i);
            s_loc = s(j);
            indat_(Y_loc_ptr,&r_loc,&s_loc,&hr,&hs,&m2p5,&I,&time);
            for(int ids = 0; ids <= m2p5; ids ++){
                for(int idr = 0; idr <= m2p5; idr ++){
