...
void SetUp::compute_initial_data(Darray4 &u, Darray4 &v){
    double scale_r, scale_s;
    double r_part;
    int case_r, case_s;
    for(int i = 0; i <= nr; i++){
        for(int idr = 0; idr <= m; idr++){
            scale_r = pow(2*M_PI*hr,idr)/tgamma(idr+1);
            case_r = idr%4;
            if(case_r == 0){
                r_part = scale_r*sin(2*M_PI*r(i));
            }
            else if(case_r == 1){
                r_part = scale_r*cos(2*M_PI*r(i));
            }
            else if(case_r == 2){
                r_part = -scale_r*sin(2*M_PI*r(i));
            }
            ...
            for(int j = 0; j <= ns; j++){
                for( int ids = 0; ids <= m; ids++){
                    scale_s = pow(2*M_PI*hs,ids)/tgamma(ids+1);
                    case_s = ids%4;
                    if(case_s == 0){
                        u(idr,ids,i,j) = r_part*scale_s*sin(2*M_PI*s(j));
                    }
                    else if(case_s == 1){
                        u(idr,ids,i,j) = r_part*scale_s*cos(2*M_PI*s(j));
                    }
                    else if(case_s == 2){
                        u(idr,ids,i,j) = -r_part*scale_s*sin(2*M_PI*s(j));
                    }
