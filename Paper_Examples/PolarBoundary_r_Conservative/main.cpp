...
int main(){
	for(int m = 2; m <= 3; m++){
		...
		for(int n = base; n <= nmax; n += 5){
		    ...
		    nsteps = ceil(T/dt);
		    dt = T/nsteps;
			SetUp.compute_initial_data(u,0,nr,0,ns,SetUp.r,SetUp.s,initial);
			SetUp.compute_initial_data(ud,1,nr,1,ns,SetUp.rd,SetUp.sd,-0.5*dt);
		    ...
		    for(int steps = 1; steps <= 1; steps++){
				Hermite.interpolate_pd(u);
				for(int i = 1; i <= nr; i++){
					for(int j = 1; j <= ns; j++){
						Hermite.recursion_curvilinear(Hermite.ud_interp,i,j,
							SetUp.a01d,SetUp.a10d,SetUp.a11d,SetUp.a12d,SetUp.a22d,ud,SetUp.hr,SetUp.hs,dt);
					}
				}
				Hermite.interpolate_dp(ud);
				Hermite.BoundaryPeriodic(ud);
				Hermite.boundaryConditions(m,SetUp.hr,SetUp.hs,ud,SetUp.a01,SetUp.a10,
					SetUp.a11,SetUp.a12,SetUp.a22);
				for(int i = 0; i <= nr; i++){
					for(int j = 0; j <= ns; j++){
						Hermite.recursion_curvilinear(Hermite.u_interp,i,j,
							SetUp.a01,SetUp.a10,SetUp.a11,SetUp.a12,SetUp.a22,u,SetUp.hr,SetUp.hs,dt);
					}
				}
				// cout << "step = " << steps*dt << endl;
		    }
		    Hermite.interpolate_pd(u); // Interpolate primal to dual
