...
void Hermite::boundaryConditions(int m,double hr,double hs,Darray4 &u,Darray4 &v, 
    ...
    for(int j = 1; j <= ns - 1; j++){
        ...
        for(int ids = 0; ids <= m-1; ids++){
            for(int idr = 0; idr <= m-1; idr++){
                rhs_v((2*m)*(2*m)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,1,j+1);
                offset++;
                rhs_v((2*m)*(2*m)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,1,j);
                offset++;
            }
        }
        bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_v_ptr,m_minus);
        sprintf(fName, "Mv%0.5d.ext",j);
        // outPutMat(Mat_u,0,(2*m)*(2*m)-1,0,(2*m)*(2*m)-1,fName);
        // columnScale(Mat_v,rhs_v,m_minus);
        // Equilibriate
        COUNT = 0;
        ...
        for(int ids = 0;ids<=2*m-1;ids++){
            for(int idr = 0;idr<=2*m-1;idr++){
                v_interp(idr,ids,0,j) = rhs_v(COUNT);
                COUNT++;
            }
        }
        ...
        for(int ids = 0; ids <= m-1; ids++){
            for(int idr = 0; idr <= m-1; idr++){
                rhs_v((2*m)*(2*m)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,nr,j+1);
                offset++;
                rhs_v((2*m)*(2*m)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,nr,j);
                offset++;
            }
        }
        bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_v_ptr,m_minus);
        ...
        for(int ids = 0;ids<=2*m-1;ids++){
            for(int idr = 0;idr<=2*m-1;idr++){
                v_interp(idr,ids,nr,j) = rhs_v(COUNT);
                COUNT++;
            }
        }
    }
    ...
    for(int idr = 0; idr <= 1; idr++){
        for(int ids = 0; ids <= 1; ids++){
            a01(idr,ids) = A01(idr,ids,0,0);
            a10(idr,ids) = A10(idr,ids,0,0);
            a11(idr,ids) = A11(idr,ids,0,0);
            a12(idr,ids) = A12(idr,ids,0,0);
            a22(idr,ids) = A22(idr,ids,0,0);
        }
    }
    // Boundary Conditions for u
    rhs_u.set_value(0.0);
    ...
    for(int ids = 0;ids<=m;ids++){
        for(int idr = 0;idr<=m;idr++){
            rhs_u((2*m+2)*(2*m+2)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*u(idr,ids,1,1);
            offset++;
            rhs_u((2*m+2)*(2*m+2)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*u(idr,ids,1,ns);
            offset++;
        }
    }
    bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_u_ptr,m);
    ...
    for(int ids = 0;ids<=2*m+1;ids++){
        for(int idr = 0;idr<=2*m+1;idr++){
            u_interp(idr,ids,0,0) = rhs_u(COUNT);
            u_interp(idr,ids,0,ns) = rhs_u(COUNT); // Use periodicity
            COUNT++;
        }
    }
    ...
    for(int ids = 0; ids <= m-1; ids++){
        for(int idr = 0; idr <= m-1; idr++){
            rhs_v((2*m)*(2*m)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,1,1);
            offset++;
            rhs_v((2*m)*(2*m)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,1,ns);
            offset++;
        }
    }
    bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_v_ptr,m_minus);
    ...
    for(int ids = 0;ids<=2*m-1;ids++){
        for(int idr = 0;idr<=2*m-1;idr++){
            v_interp(idr,ids,0,0) = rhs_v(COUNT);
            v_interp(idr,ids,0,ns) = rhs_v(COUNT); // Use Periodicity
            COUNT++;
        }
    }
    ...
    for(int idr = 0; idr <= 1; idr++){
        for(int ids = 0; ids <= 1; ids++){
            a01(idr,ids) = A01(idr,ids,nr,0);
            a10(idr,ids) = A10(idr,ids,nr,0);
            a11(idr,ids) = A11(idr,ids,nr,0);
            a12(idr,ids) = A12(idr,ids,nr,0);
            a22(idr,ids) = A22(idr,ids,nr,0);
        }
    }
    // Boundary Conditions for u
    rhs_u.set_value(0.0);
    ...
    for(int ids = 0;ids<=m;ids++){
        for(int idr = 0;idr<=m;idr++){
            rhs_u((2*m+2)*(2*m+2)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*u(idr,ids,nr,1);
            offset++;
            rhs_u((2*m+2)*(2*m+2)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*u(idr,ids,nr,ns);
            offset++;
        }
    }
    bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_u_ptr,m);
    ...
    for(int ids = 0;ids<=2*m+1;ids++){
        for(int idr = 0;idr<=2*m+1;idr++){
            u_interp(idr,ids,nr,0) = rhs_u(COUNT);
            u_interp(idr,ids,nr,ns) = rhs_u(COUNT); // Copy since periodic
            COUNT++;
        }
    }
    ...
    for(int ids = 0; ids <= m-1; ids++){
        for(int idr = 0; idr <= m-1; idr++){
            rhs_v((2*m)*(2*m)/2+offset) = tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,nr,1);
            offset++;
            rhs_v((2*m)*(2*m)/2+offset) =  tgamma(idr+1)*tgamma(ids+1)*v(idr,ids,nr,ns);
            offset++;
        }
    }
    bcMat(rA,rB,sA,sB,hr,hs,a01_ptr,a10_ptr,a11_ptr,a12_ptr,a22_ptr,Mat_v_ptr,m_minus);
    ...
    for(int ids = 0;ids<=2*m-1;ids++){
        for(int idr = 0;idr<=2*m-1;idr++){
            v_interp(idr,ids,nr,0) = rhs_v(COUNT);
            v_interp(idr,ids,nr,ns) = rhs_v(COUNT); // Copy since periodic
            COUNT++;
        }
    }
}
