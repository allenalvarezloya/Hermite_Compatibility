...
void Hermite::recursion_curvilinear(Darray4 &u_int,int i,int j,
    ...
    curviLaplacian(hr,hs,m,dt,a01_recursion,a10_recursion,a11_recursion,a12_recursion,a22_recursion,u_loc);
    for(int k = 1; k <= m; k++){
        for(int ids = 0; ids <= m2p1; ids++){
            for(int idr = 0; idr <= m2p1; idr++){
                lapSum(idr,ids) += u_loc(idr,ids)/tgamma(2*k+1);
            }
        }
        curviLaplacian(hr,hs,m,dt,a01_recursion,a10_recursion,a11_recursion,a12_recursion,a22_recursion,u_loc);
    }
...
//--------------------------------------------------
void Hermite::curviLaplacian(double hx,double hy,int m,double dt,Darray2 &a01,Darray2 &a10,Darray2 &a11,Darray2 &a12,Darray2 &a22,Darray2 &u){
    int m2p1 = 2*m+1;
    ...
    for(int j = 0; j <= m2p1; j++){
        for(int i = 0; i <= m2p1 - 2; i++){
            uxx(i,j) = double(i+1)*double(i+2)*u(i+2,j)*pow(0.5*dt/hx,2);
        }
    }
    // uyy
    for(int i = 0; i <= m2p1; i++){
        for(int j = 0; j <= m2p1 - 2; j++){
            uyy(i,j) = double(j+1)*double(j+2)*u(i,j+2)*pow(0.5*dt/hy,2);
        }
    }
    // ux 
    for(int j = 0; j <= m2p1; j++){
        for(int i = 0; i <= m2p1 - 1; i++){
            ux(i,j) = double(i+1)*u(i+1,j)*(0.5*dt/hx)*(0.5*dt);
        }
    }
    // uy
    for(int i = 0; i <= m2p1; i++){
        for(int j = 0; j <= m2p1 - 1; j++){
            uy(i,j) = double(j+1)*u(i,j+1)*(0.5*dt/hy)*(0.5*dt);
        }
    }
    // uxy
    for(int i = 0; i <= m2p1 - 1; i++){
        for(int j = 0; j <= m2p1 - 1; j++){
            uxy(i,j) = double(i+1)*double(j+1)*u(i+1,j+1)*(0.5*dt/hx)*(0.5*dt/hy);
        }
    }
