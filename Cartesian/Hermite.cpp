...
void Hermite::oversample(Darray4 &u,double time,Darray1 &errors1,Darray1 &errors2,Darray1 &errorsinf,int COUNT){
    ...
    for (int i = 0; i <= nsample_r*nr; i++){
        for (int j = 0; j <= nsample_s*ns; j++){
            L1error +=  abs(U(i,j) - sin(2*M_PI*(Hr*i))*sin(2*M_PI*(Hs*j))*cos(time*pow(2,1.5)*M_PI));
            L2error += pow(U(i,j) - sin(2*M_PI*(Hr*i))*sin(2*M_PI*(Hs*j))*cos(time*pow(2,1.5)*M_PI),2.0);
            if(abs(U(i,j) - sin(2*M_PI*(Hr*i))*sin(2*M_PI*(Hs*j))*cos(time*pow(2,1.5)*M_PI)) > Linferror){
                Linferror = abs(U(i,j) - sin(2*M_PI*(Hr*i))*sin(2*M_PI*(Hs*j))*cos(time*pow(2,1.5)*M_PI));
            }
            Uerror(i,j) = abs(U(i,j) - sin(2*M_PI*(Hr*i))*sin(2*M_PI*(Hs*j))*cos(time*pow(2,1.5)*M_PI));
        }
    }
    L1error *= Hr*Hs;
    L2error *= Hr*Hs;
    ...
    // Uerror.writeToFile(fName,0,nsample_r*nr,0,nsample_s*ns);
}
