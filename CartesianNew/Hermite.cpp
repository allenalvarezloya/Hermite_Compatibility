...
void Hermite::boundaryConditions(int m,double hr,double hs,Darray4 &u,Darray4 &v, 
    ...
    for(int j = 1; j <= ns - 1; j++){
        ...
        // for(int i = 0; i <= (2*m+2)*(2*m+2)-1; i++){
        //     cout << "j = " << j << " i " << i << " COL(i) = " << COL(i) << " ROW(i) = " << ROW(i) << endl;
        //  }
        COUNT = 0;
        for(int j = 0; j <= (2*m+2)*(2*m+2)-1;j++){
            for(int i = 0; i <= (2*m+2)*(2*m+2)-1;i++){
        ...
        sprintf(fName, "R%0.5d.ext",j);
        outPutVec(ROW,0,(2*m+2)*(2*m+2)-1,fName);
        sprintf(fName, "C%0.5d.ext",j);
        outPutVec(COL,0,(2*m+2)*(2*m+2)-1,fName);
