...
void Hermite::boundaryConditions(int m,double hr,double hs,Darray4 &u,Darray4 &v, 
    ...
    columnScale(Mat_u,rhs_u,m);
    // char fName[100];
    // sprintf(fName, "M3.ext");
    // Mat_u.writeToFile(fName,0,(2*m+2)*(2*m+2)-1,0,(2*m+2)*(2*m+2)-1);
    // cout << Mat_u(63,63) << "Here" << endl;
    dgetrf_(&Mboundary,&Nboundary,Mat_u_ptr,&LDAboundary,IPIV,&INFO);                  // Factor matrix
    dgetrs_(&no,&Nboundary,&ONE,Mat_u_ptr,&Nboundary,IPIV,rhs_u_ptr,&Nboundary,&INFO); // Solve system
    COUNT = 0;
    for(int ids = 0;ids<=2*m+1;ids++){
...
//--------------------------------------------------
void Hermite::oversample(Darray4 &u,double time,Darray1 &errors1,Darray1 &errors2,Darray1 &errorsinf,int COUNT){
    // Oversample 
    ...
    Darray2 U;
    U.define(0,nsample_r*nr,0,nsample_s*ns);
    U.set_value(0.0);
    Darray2 Uerror;
    Uerror.define(0,nsample_r*nr,0,nsample_s*ns);
    Uerror.set_value(0.0);
    double rloc, sloc;
    for(int i = 1;i<=nr;i++){
        for(int j = 1;j<=ns;j++){
    ...
    double hr, hs; 
    hr = 1.0/double(nr);
    Hr = 1.0/double(nr*nsample_r);
    hs = 1.0/double(ns);                    
    Hs = 1.0/double(ns*nsample_s);      
    double L1error, L2error, Linferror;
    L1error = 0.0; 
    L2error = 0.0;
    Linferror = 0.0;
    for (int i = 0; i <= nsample_r*nr; i++){
        for (int j = 0; j <= nsample_s*ns; j++){
            L1error +=  abs(2*M_PI*alpha*(alpha*(Hr*i)+gamma)*(U(i,j) - Bessel(3,(alpha*Hr*i+gamma)*k33)*cos(3*2*M_PI*Hs*j)*cos(k33*time)));
            L2error += 2*M_PI*alpha*(alpha*(Hr*i)+gamma)*pow(U(i,j) - Bessel(3,(alpha*Hr*i+gamma)*k33)*cos(3*2*M_PI*Hs*j)*cos(k33*time),2);
            Uerror(i,j) = abs(U(i,j) - Bessel(3,(alpha*Hr*i+gamma)*k33)*cos(3*2*M_PI*Hs*j)*cos(k33*time));
            if (abs(U(i,j) - Bessel(3,(alpha*Hr*i+gamma)*k33)*cos(3*2*M_PI*Hs*j)*cos(k33*time)) > Linferror){
                Linferror = abs(U(i,j) - Bessel(3,(alpha*Hr*i+gamma)*k33)*cos(3*2*M_PI*Hs*j)*cos(k33*time));
            }
        }
    }
    L1error *= Hr*Hs;
    ...
    char fName[100];
    sprintf(fName, "u%01d.ext", COUNT);
    outPutMat(Uerror,0,nsample_r*nr,0,nsample_s*ns,fName);
}


void Hermite::M1(double rA,double rB,double sA,double sB,double hr,double hs,
...
//--------------------------------------------------
void Hermite::interpolate_dp_ext(Darray4 &ud, Darray4 &vd){
    // This subroutine interpolates from dual to priaml
