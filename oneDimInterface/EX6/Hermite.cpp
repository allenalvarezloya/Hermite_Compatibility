...
//------------------------------------------
//------------------------------------------ 
void Hermite::boundaryConditions(int m, double hr,Darray2 &u, Darray2 &v,double t){
    int m2p1 = 2*m+1;
    // LAPACK info for u
    ...
    rhs_u.set_value(0.0);
    rhs_v.set_value(0.0);
    boundaryValues(m,hr,t,rhs_u,rhs_v);
    for(int idr = 0; idr <= m; idr++){
    ...
    for(int idr = 0; idr <= m-1; idr++){
        rhs_v(m+idr) = tgamma(idr+1)*v(idr,1);
    }
    ...
    for(int idr = 0; idr <= m-1; idr++){
        rhs_v(m+idr) = tgamma(idr+1)*v(idr,nr);
    }
    bcMat(rA,hr,Mat_v_ptr,m-1);
    dgetrf_(&Mboundary_v,&Nboundary_v,Mat_v_ptr,&LDAboundary_v,IPIV_v,&INFO);
    dgetrs_(&no,&Nboundary_v,&ONE,Mat_v_ptr,&Nboundary_v,IPIV_v,rhs_v_ptr,&Nboundary_v,&INFO);
    for(int idr = 0; idr <= 2*m-1; idr++){
        v_interp(idr,nr) = rhs_v(idr);
    }
}
//------------------------------------------
//------------------------------------------ 
void Hermite::boundaryValues(int m,double h,double t,Darray1 &RHSu,Darray1 &RHSv){
    double pi = 3.1415926535897932385;
    for(int idr = 0; idr <= m; idr ++){
        RHSu(idr) = pow(-1.0,idr)*pow(2*pi*h,2*idr)*sin(-2*pi*t);
    }
    for(int idr = 0; idr <= m-1; idr ++){
        RHSv(idr) = 2*pi*pow(-1.0,idr+1)*pow(2*pi*h,2*idr)*cos(-2*pi*t);
    }
}
//------------------------------------------
//------------------------------------------ 
