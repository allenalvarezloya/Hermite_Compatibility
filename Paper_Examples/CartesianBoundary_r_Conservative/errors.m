...
for k = 1:N
    ...
    for j = 1:ns
        for i = 1:nr
            Uactual(i,j) = cos(2^1.5*pi*T)*sin(2*pi*X(i,j))*sin(2*pi*Y(i,j));
            L1 = L1 + abs(U(i,j) - Uactual(i,j));
            L2 = L2 + (U(i,j) - Uactual(i,j)).^2;
...
loglog(H(1:3:end),L2_m2(1:3:end),H(1:3:end),L2_m3(1:3:end),H(1:3:end),H(1:3:end).^4,'--k',H(1:3:end),0.25*H(1:3:end).^6,'--k','LineWidth',2)
set(gca,'FontSize',16)
